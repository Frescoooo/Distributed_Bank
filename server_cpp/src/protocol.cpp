...
static inline bool getBE64(const std::vector<uint8_t>& b, size_t& off, uint64_t& out) {
  ...
  out = 0;
  for (int i = 0; i < 8; i++) out = (out << 8) | uint64_t(b[off+i]);
  off += 8;
  return true;
}
...
void putString(std::vector<uint8_t>& b, const std::string& s) {
  if (s.size() > 65535) return; // keep simple
  putU16(b, uint16_t(s.size()));
  b.insert(b.end(), s.begin(), s.end());
}

void putPassword16(std::vector<uint8_t>& b, const std::string& s) {
  // fixed 16 bytes; if >16, we'll truncate here (server will reject if it cares)
  std::string t = s;
...
bool getU16(const std::vector<uint8_t>& b, size_t& off, uint16_t& out) { return getBE16(b, off, out); }
bool getU32(const std::vector<uint8_t>& b, size_t& off, uint32_t& out) { return getBE32(b, off, out); }
bool getU64(const std::vector<uint8_t>& b, size_t& off, uint64_t& out) { return getBE64(b, off, out); }
bool getI32(const std::vector<uint8_t>& b, size_t& off, int32_t& out) {
  uint32_t u; if (!getU32(b, off, u)) return false; out = int32_t(u); return true;
}
bool getDouble(const std::vector<uint8_t>& b, size_t& off, double& out) {
  uint64_t bits; if (!getU64(b, off, bits)) return false;
